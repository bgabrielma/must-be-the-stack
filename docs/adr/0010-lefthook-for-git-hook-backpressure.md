...
## Consequences
One more dev-environment dependency (Lefthook binary) needs installing in `.devcontainer/` alongside Ruby/Node so hooks work inside the container, not just on a host machine that happens to have it. Hook config (`lefthook.yml`) lives at the monorepo root and must be kept in sync as new lint/test commands are added to either `packages/api` or `apps/web`.
